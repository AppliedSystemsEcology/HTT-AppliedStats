...
TomatoThripsDamageRS <- read.xlsx(xlsxFile = "HTTFruitRating_2025.xlsx", sheet = 2)
TomatoThripsDamageSEAREC <- read.xlsx(xlsxFile = "HTTFruitRating_2025.xlsx", sheet = 1)
...
for (i in seq_along(temperature_list_RS)) {
  # read excel files
  xlsxFile <- read.xlsx(paste0(temperature_RS_directory,"/",temperature_list_RS[i]))
   # Add "temp_" prefix to file name
  new_filename <- paste0("temp_", temperature_list_RS[i])
  # write into new directory as excel files
  write.xlsx(xlsxFile, paste0(appliedstats_filepath_temp_RS, "/", new_filename)
  )
}
...
for (i in seq_along(temperature_list_SEAREC)) {
  # read excel files
  xlsxFile <- read.xlsx(paste0(temperature_SEAREC_directory,"/",temperature_list_SEAREC[i]))
   # Add "temp_" prefix to file name
  new_filename <- paste0("temp_", temperature_list_SEAREC[i])
  # write into new directory as excel files
  write.xlsx(xlsxFile, paste0(appliedstats_filepath_temp_SEAREC, "/", new_filename)
  )
}
...
TomatoThripsDamageRS <- read.xlsx("TomatoThripsDamageRS.xlsx")
TomatoThripsDamageSEAREC <- read.xlsx("TomatoThripsDamageSEAREC.xlsx")

# Load ground cover ID data from local environment
GroundCoverID <- read.xlsx("Site_GroundCover_2025.xlsx")
...
# read.xlsx() to pull C or F from temperature column
