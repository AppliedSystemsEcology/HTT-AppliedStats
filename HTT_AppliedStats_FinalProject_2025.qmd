...
Temperature <- Temperature_raw %>%
  left_join(GroundCoverID %>%
   select(ground_cover, plot_ID), by = "plot_ID")
...
# Other predictor variables:  site (categorical-nominal), date (not sure type of data)
...
damage.model <- polr(damage_rating~site*ground_cover*date, data=TomatoThripsDamage, Hess=TRUE)
...
# null model
temperature.null <- lm(temperature_c~1, data = Temperature)
# full model
temperature.full <- lm(temperature_c~sensor_placement*ground_cover*site*date_time, data = Temperature)
