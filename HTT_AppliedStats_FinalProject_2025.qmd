...
temperature_df_list_purrr <- map2( 
  ...
  # delete dummy temp column
    ...
    mutate(julian_day = yday(date),
      day_night = case_when(
        between(time, as_hms("06:00:00"), as_hms("20:00:00")) ~ "daytime",
        TRUE ~ "nighttime")
  )
)
...
# models the relationship between the predictors and an outcome variable with there or more categories that have a natural order
# polr () - proportional odds logistic  regression
# Hess=TRUE for the model to rreturn the observed information matrix from optimizatio which is used to get standard errors
...
# Proposed model
# temperature~site*sensor_placement*ground_cover*date_time
# If sensor placement is sig, then maybe make two different models; one for above and one for below?
