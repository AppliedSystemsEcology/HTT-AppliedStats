...
# add column for groundcover
    separate(col = sensor_ID, into = c("temp", "site", "plot_ID_row", "sensor_placement"), sep = "_") %>%
  # delete dummy temp column
    select(!(temp)) 
    #%>%
    # separate plot_ID and row (this isnt working with no deliminator)
    #separate(col = plot_ID_row, into = c("plot_ID", "row"))

    # lubridate on date and time columns (not done yet)
    )
